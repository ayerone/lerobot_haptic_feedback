# Modified by github: ayerone
#
# to provide feedback to the teleoperator
...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            ...
            feedback = teleop.send_feedback(obs)

            # Applies a pipeline to the raw teleop action, default is IdentityProcessor
            act_processed_teleop = teleop_action_processor((act, obs))

            sensor_force = round(obs["sensor.force"])
            gripper_pos = round(obs["gripper.pos"])
            teleop_pos = round(teleop._gimbal_position)
            
            print(f"{sensor_force=} {gripper_pos=} {teleop_pos=} {feedback=} ")
            move_cursor_up(1)
